...
def analyse_score(dice_value_list, old_score, player):
    ...
    if total_dice_remain_calcul == 0 and turn_is_scored == True:
        ...
        constants.STACK_BONUS = 0
        print("perdu sale chien")


    print("il te reste " + str(constants.THROW_DICE_COUNTER) + " dés a lancer")
...
def game():
    ...
    for player in players:
        ...
        dice_value_list = roll_dice_set(constants.THROW_DICE_COUNTER)
        ...
        # analyse_bonus_score(roll_dice_set(constants.THROW_DICE_COUNTER, player), player)
